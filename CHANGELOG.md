...
- **`apt-get install` of a cached package works offline**: completing the offline story, the proxy now serves an already-cached `.deb` even after a restart with no `apt-get update` this session. Previously the in-memory index (which maps a package URL to its SHA256) was only warmed by live `Packages` requests, so `apt-get install` — which doesn't re-fetch indices — couldn't resolve a cached package's hash and fell through to an uncached passthrough that failed offline. The proxy now warms the index once from cached `Packages` metadata on the first unresolved package request, then serves the hit entirely from disk with no network. And when a package is genuinely not cached and the node is offline (no internet and no mDNS peers), the request now fails fast with `503` instead of grinding through the doomed fleet/DHT/P2P/mirror chain. (`lan_only` mirror suppression in the download path remains a follow-up.)
