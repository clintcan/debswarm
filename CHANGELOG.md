...
- **Optional relayed transfer for symmetric-NAT'd peers.** DCUtR cannot hole-punch through a symmetric NAT, so when *both* peers are symmetric-NAT'd they could never transfer peer-to-peer and always fell back to the mirror. A new `[network] relayed_transfer_max_bytes` (default `0`, off) lets such a pair exchange **small** packages over the circuit-relay connection instead. This is safe by construction — it is a bandwidth/cost choice, not a security one: every relayed byte is still SHA256-verified against the signed index (a relay cannot poison the swarm), and the relay carries the end-to-end-encrypted libp2p stream, so it is a blind pipe that can only drop or delay, never read or forge. When enabled, a relayed source is bounded by `min(relayed_transfer_max_bytes, the relay's per-circuit buffer_size)` and joins the existing P2P-vs-mirror race, so it never delays a peer that can reach the mirror; a direct path is always preferred. Disabled by default and best suited to private (PSK) swarms; a relay opts in to carrying data by raising `relay_limits.buffer_size`. New metrics: `debswarm_bytes_from_relay_total` and `debswarm_relayed_transfer_total{result}`. Design: `docs/design/relay-data-fallback.md`.
...
### Added
- **Flat-layout repositories are now GPG-verified.** Daemon-side signature verification previously covered only dist-layout repositories (those with a `/dists/<suite>/` tree); a flat-layout repository was reported `no-dist` and never verified. debswarm now anchors a flat repo's index to the signature-verified `Release`/`InRelease` that sits in the same directory (and lists index files by bare name), exactly as it does for a dist repo. A flat repository whose `Release` carries a modern (v4) OpenPGP signature debswarm can discover a key for therefore gets full `auto`/`enforce` protection with no extra configuration. A flat repo with no cached, verifiable `Release` falls back to the indecisive `no-release` result (served-and-flagged under `warn`/`auto`).
  - **Note on `pkgs.k8s.io`:** the Kubernetes repositories currently sign their `InRelease` with a **legacy v3 OpenPGP signature**, which the Go OpenPGP library (like the modern OpenPGP spec) does not accept — only GnuPG still does, for backward compatibility. debswarm therefore cannot verify `pkgs.k8s.io` and reports `no-release`: it is served-and-flagged under the default `auto` (unchanged behavior), and `enforce` needs it in `verify_exempt_hosts`. This is a signature-format limitation upstream, not a debswarm configuration issue.
- **Flat-layout `Acquire-By-Hash` indices are now recognized and verified.** A flat repo whose `Release` sets `Acquire-By-Hash: yes` serves its `Packages` index under `by-hash/SHA256/<hash>` with no `binary-*/` path to classify by; debswarm previously treated those as an unclassified passthrough (unverified). They are now recognized as Packages indices, cached, and verified against the signed `Release` like any other index (`i18n`/`cnf`/`dep11` by-hash remain excluded as non-Packages).
- **`enforce` fetches a missing `Release` on demand** instead of falsely refusing. Verification reads the signed `Release` from the metadata cache, which APT normally populates by fetching it before any `Packages` request — but if a client's APT already holds a current `InRelease` (so debswarm's conditional GET relays a `304` with no body) or the metadata cache was cleared, `enforce` had no `Release` to verify against and refused a verifiable index. It now fetches the `Release` directly from the mirror once (deduplicated across concurrent requests, and negatively cached briefly on failure) so a verifiable index is served. This fetch is **`enforce`-only** — `auto`/`warn` serve regardless, so their (default) serving path takes on no extra network I/O.
