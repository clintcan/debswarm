...
- **Upstream Signature Verification** (v1.34+, optional) - The daemon can verify each repository's GPG-signed `Release` against a trusted keyring (auto-discovered from APT's keyrings) and each `Packages` index against it, anchoring every `.deb` hash to GPG instead of the mirror's word. `warn` (default) reports failures without changing what `apt` sees; `auto` refuses an index it *can* verify but that fails, while still serving repos it can't verify; `enforce` refuses any unverified index — closing the gap for `[trusted=yes]` repos, `dpkg -i` from the cache, and P2P-seeded packages
