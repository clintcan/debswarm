...
- **Upstream Signature Verification** (v1.34+) - The daemon verifies each repository's GPG-signed `Release` against a trusted keyring (auto-discovered from APT's keyrings) and each `Packages` index against it, anchoring every `.deb` hash to GPG instead of the mirror's word. `auto` (default) refuses an index it *can* verify but that fails, while still serving repos it can't verify; `warn` reports failures without changing what `apt` sees; `enforce` refuses any unverified index; `off` disables it — closing the gap for `[trusted=yes]` repos, `dpkg -i` from the cache, and P2P-seeded packages
...
**`apt`'s own signature verification is always your baseline protection against a tampered mirror, and debswarm always preserves it** — it passes fetched bytes through unmodified, and `apt` performs its normal client-side verification (the GPG signature on the `Release` file, then the hash chain down to each `.deb`) exactly as it would without a proxy. That end-to-end guarantee is intact in every mode.

Since v1.34+, debswarm *also* performs its **own** daemon-side GPG verification, **on by default**: `[security] verify_upstream_signatures` verifies each repository's signed `Release` against a trusted keyring (auto-discovered from APT's keyrings) and each `Packages` index against it. The default, `auto`, refuses an index it *can* verify but that fails — real anti-tampering protection at the daemon — while still serving (and flagging with `X-Debswarm-Unverified`) a repo it *cannot* verify; `warn` only observes and reports; `enforce` refuses any index it cannot vouch for; `off` disables it. For a repository debswarm cannot verify, `apt`'s client-side check remains the actual guarantee. See [SECURITY.md](SECURITY.md#upstream-signature-verification-optional-v134).
